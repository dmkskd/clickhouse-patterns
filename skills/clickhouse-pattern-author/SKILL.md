...
5. Write a concise description that answers what happens and why the pattern exists. Add concrete `tradeoffs.benefits` and `tradeoffs.limitations`.
6. Link primary upstream documentation or a precise issue when it supports a compatibility or behavior claim. To link another pattern, choose the right mechanism for where the reader is: `[[slug|label]]` inline in the description when the prose names it, `related_patterns` for a standing pointer, a `group.yaml` advisory for a family-wide caution. See [references/pattern-model.md](references/pattern-model.md).
